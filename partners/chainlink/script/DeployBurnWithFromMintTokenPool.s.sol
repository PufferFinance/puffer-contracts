...
import { BurnWithFromMintTokenPool } from "@chainlink/contracts-ccip/src/v0.8/ccip/pools/BurnWithFromMintTokenPool.sol";
import { BurnMintERC677 } from "@chainlink/contracts-ccip/src/v0.8/shared/token/ERC677/BurnMintERC677.sol";
import { IBurnMintERC20 } from "@chainlink/contracts-ccip/src/v0.8/shared/token/ERC20/IBurnMintERC20.sol";

contract DeployBurnWithFromMintTokenPool is Script {
  function run() external {
    ...
    // Deploy the BurnWithFromMintTokenPool contract associated with the token
    BurnWithFromMintTokenPool tokenPool = new BurnWithFromMintTokenPool(
      token,
    ...
    console.log(
      "Burn With From Mint token pool deployed to:",
      address(tokenPool)
    );
    ...
    string memory poolFileName = string(
      abi.encodePacked("./script/output/deployedTokenPool_", chainName, ".json")
    );
