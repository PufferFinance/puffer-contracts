...
As Puffer's permissionless node operators deploy their validators, they set their _withdrawal credentials_ to point to a `PufferModule's` `EigenPod` to participate in restaking.
...
To reduce gas-costs for node operators, each merkle proof will allow them to withdraw the rewards for _all_ of their registered validators over the rewards period.
